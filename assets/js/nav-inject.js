...
<footer id="footer">
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    import('/assets/js/supabase.js?v=ba3abb76').then(async ({ getNavMenu }) => {
      try {
    ...
    if (newsletterBtn && newsletterInput) {
      newsletterBtn.addEventListener('click', async () => {
        ...
        try {
          const { subscribeToNewsletter } = await import('/assets/js/supabase.js?v=ba3abb76');
    ...
    if (partnersMount) {
      import('/assets/js/supabase.js?v=ba3abb76').then(async ({ getPartners }) => {
        try {
